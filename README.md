...
Canonical React + Vite template for Timbal **chat** apps. The UI lives in [`@timbal-ai/timbal-react`](https://www.npmjs.com/package/@timbal-ai/timbal-react) (`^1.0.0`); this repo is a thin shell composers and teams clone.
...
- [ ] `@timbal-ai/timbal-react` pinned to a published version (e.g. `^1.0.0`); use `file:../timbal-react` + `dev:linked` only for local library dev
