...
**Light is the default in this blueprint.** Before first paint, `index.html` reads the `timbal:theme` preference and toggles `class="dark"` on `<html>` only when the user explicitly chose dark; nothing is ever written there. The vendored `ThemeToggle` is patched (`boardui-sync.mjs`, "theme default" step) to (a) use the same `timbal:theme` key — builds before 2026-09-14 auto-wrote their default under upstream's `boardui:theme`, and on a shared preview origin that stale value came back as a "preference" in every later app — and (b) fall back to whatever class `<html>` carries when storage is empty or blocked, so page and toggle never disagree. Only a click on the toggle persists. Dark stays a first-class mode with every token bound. To ship dark-first for a product, change the `mode` default in that one script.
...
| --- | --- | --- |
| Surfaces (dark) | page `neutral-950`; BoardUI's own ladder above it — trays `neutral-900`, tiles `neutral-800`, control borders `neutral-700`, separators `neutral-800` | BoardUI's dark page is `neutral-925`, one invisible step from its `900` trays; dropping the page to 950 makes every card read. The ladder itself is untouched so vendored blocks keep their contrast |
| Surfaces (light) | BoardUI's own: white page, `neutral-100` trays, white tiles, `neutral-200` hairlines | A `neutral-50` page was tried and read as "all grey" (250 vs 245 is not a step). The panel is the grey tray; a white tile on the white page takes a hairline |
| Borders | `@layer base` in `brand.css` defaults `border-color` to `--color-separator-border`, `--tw-ring-color` to the focus token | Tailwind v4 defaults borders to `currentColor`, so any bare `border` / `divide-y` painted a black (or, in dark, white) line — the "weird heavy borders" on generated screens |
| Status chips (light) | `--color-status-*-background` → 12–16% tint of the `-500` hue, ink `-700`/`-800` (dark keeps BoardUI's translucent `-950` fills) | BoardUI's `-200` fills are pastel slabs that shout in a dense table; the tint carries the hue, the ink carries the meaning — the same recipe dark mode already used |
...
| Type | Inter (body, UI) and JetBrains Mono (code), **self-hosted** via `@fontsource-variable/*` imported in `src/index.css`; ramp one step down (13px body), tabular numerals | No Google Fonts request: a preview sandbox or customer network that blocks it would otherwise silently swap in the system face. See Type scale |
...
| Colour | BoardUI's own hues: accent ramp, status chips (`lime` / `rose` / `yellow` / `cyan` / `purple`), chart palette, notification and calendar tints. Only the light chip *fills* are lightened (row below) | The palette is the design system's signature; seriousness comes from surfaces, density, type and radius, not from desaturating it |
...
- **Dark muted text**: BoardUI leaves `--color-text-secondary` at 500 and sets tertiary to 600, which disappears on 800/900 surfaces. `brand.css` remaps secondary→400, tertiary/placeholder→500 and icon secondary/tertiary→400/500.
- **`:root` vs `.dark` in `brand.css`**: they have the same specificity, and `brand.css` loads after `theme.css`, so a token set in `:root` there beats BoardUI's `.dark` value. Anything you override for light must be restated under `.dark` (the status chips are the worked example).
