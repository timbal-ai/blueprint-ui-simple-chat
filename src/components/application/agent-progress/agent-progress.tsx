...
export interface AgentProgressProps {
  ...
  onFinished?: () => void;
  /**
   * Freezes the demo clock: no step advances and `onFinished` doesn't fire
   * while true. The module keeps its current state and resumes when it flips
   * back — for holding a showcase instance that has scrolled out of view.
   */
  paused?: boolean;
  className?: string;
}
...
export function AgentProgress({
  ...
  useEffect(() => {
    if (!processingStarted || complete || paused) return;
  ...
  }, [complete, completedCount, paused, processingStarted, progressSteps.length, safeStepDuration]);
