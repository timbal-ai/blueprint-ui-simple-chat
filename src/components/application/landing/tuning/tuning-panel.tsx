...
import type { TuningControl, TuningGroup } from "@/components/application/landing/tuning/tuning-types";
export type { TuningControl, TuningGroup } from "@/components/application/landing/tuning/tuning-types";
...
export function TuningPanel({ sections }: { sections: TuningSection[] }) {
  ...
  return (
    <div className="fixed right-4 bottom-4 z-[60] flex max-h-[calc(100vh-2rem)] w-[336px] flex-col rounded-2xl border border-white/12 bg-[#0a0c11]/95 text-white shadow-2xl backdrop-blur-xl">
      <header className="flex shrink-0 flex-wrap items-center gap-2 border-b border-white/10 px-3.5 py-3">
        <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1 rounded-lg bg-white/8 p-0.5">
        ...
        {active.presets ? (
          <select
            ...
            className="ml-auto min-w-0 shrink rounded-lg border border-white/15 bg-white/5 px-2 py-1 text-[12px] text-white/90 outline-none"
          >
                ...
                <label key={control.key} className="block" title={control.hint}>
                  <span className="flex items-baseline justify-between gap-2">
                    ...
                    <span className="font-mono text-[11px] text-white/45 tabular-nums">
                      {control.type === "color"
                        ...
                        : control.type === "select"
                          ? (control.options.find((o) => o.value === Number(active.values[control.key]))?.label ?? "")
                  ...
                  {control.type === "select" ? (
                    <select
                      ...
                      className="mt-1 w-full cursor-pointer rounded-md border border-white/15 bg-white/5 px-2 py-1 text-[12px] text-white/90 outline-none"
                    >
                      {control.options.map((option) => (
                        <option key={option.value} value={option.value} className="bg-[#0a0c11]">
                          {option.label}
                        </option>
                      ))}
                    </select>
