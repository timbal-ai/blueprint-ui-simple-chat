...
import { MetricLegendList } from "@/components/blocks/interactive-charts";
...
export default function GalleryCharts() {
  return (
    <GalleryPage
      title="Charts"
      description="Recharts through ChartContainer — colors and tooltips come from DNA chart tokens. No legends, no Y-axis numbers, edge-less plots. Interactive metric CARDS (tracked bars, rings, score ring, heatmap, trend) are the BoardUI Pro components (src/components/application) — see the Home, Medical, and AI profile template pages."
    >
      <HeroMetricCard
        title="Networth"
      ...
      <div className="grid grid-cols-1 gap-4 xl:grid-cols-2">
        ...
        {/* Metric legend — the Beacon reference grammar (no Pro equivalent). */}
        <ChartCard
          title="Metric legend"
          description="blocks/interactive-charts MetricLegendList — status rows with tone pill, big number, caption, action"
          height="auto"
          bleed={false}
        >
          <MetricLegendList
            items={[
              {
                ...
                value: "50%",
                caption: "On track",
                action: <Button variant="secondary" size="small">View</Button>,
              },
              {
                id: "behind",
                tone: 6,
