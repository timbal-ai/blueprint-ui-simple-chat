...
- Pin `@timbal-ai/timbal-react` to a **published** version (e.g. `^1.0.0`); use `file:../timbal-react` + `bun run dev:linked` only for local library dev. Add `motion` if you import `motion/react`.
